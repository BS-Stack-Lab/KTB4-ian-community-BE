...
if grep -RInE 'pull_request:|merge_group:|repository_dispatch:|workflow_call:|PRODUCTION_DEPLOY_ENABLED|PRODUCTION_DISPATCH_ENABLED|BACKEND_DISPATCH_TOKEN' "${root}/.github/workflows"; then
...
if ! grep -qE '^  workflow_dispatch:$' "${root}/.github/workflows/deploy-production.yml"; then
