...
사용자가 제공한 2026-08-03~04 Console·Session Manager·외부 포트 검사 중 A
방식의 Public IPv4·HTTP 검증 결과:
...
- PASS: A 방식 기능 검증에는 EC2 Public IPv4와 HTTP 80 사용
- PASS: A 방식 검증 시 Dynu·HTTPS 미사용, 443 외부 미공개
...
## B 방식 작업 단계에서 추가한 공통 Host 진입점 검증

아래 결과는 A 방식의 Public IPv4·HTTP 검증에 포함하지 않는다. 파일은 Host
Nginx 운영 Bundle을 재사용하기 위해 `method-a` 아래에 있지만, Dynu와 HTTPS를
처음 실행하고 검증한 시점은 B 방식 작업 단계다.
...
이 B 작업 단계에서 만든 최종 공개 주소는 `https://pulse.gleeze.com/`이다.
외부 검사에서 HTTPS health는 200, 인증서 검증 결과는 정상, HTTP health는
같은 HTTPS 주소로 301 Redirect됐다. 운영 IP Update Password와 Hash는 증빙에
기록하지 않는다.
