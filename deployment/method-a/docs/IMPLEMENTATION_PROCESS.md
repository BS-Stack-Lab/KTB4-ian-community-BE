...
- Backend `3886355` (A 방식 EC2 배포 지원 추가)
- Frontend `70fed0f` (A 방식 배포를 위한 프론트엔드 준비)
...
## 11. B 방식 작업 중 공통 Host에 무료 고정 도메인과 HTTPS 추가 (2026-08-04)

위 1~10장은 `3886355` 기준의 HTTP 검증 이력을 보존한다. 2026-08-04에는
B 방식 작업을 진행하면서 다음 공개 Host 구성을 실제 EC2에 처음 추가하고 최종
검증했다. 관련 Script는 Host Nginx 설정을 재사용하기 위해 `method-a` 아래에
있지만 A 방식의 초기 Public IPv4·HTTP 검증 범위에는 포함하지 않는다.
